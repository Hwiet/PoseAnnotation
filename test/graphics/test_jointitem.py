from graphics import JointItem
...
class TestJointItem(unittest.TestCase):
    def setUp(self):
        ...
        with open("supported_models/PoseNet/schema.json") as fp:
            self.model = PoseModel(json.load(fp))
        ...
        self.item = JointItem(index, "right_ear")

    def test_index(self):
        self.assertEqual(self.item.index(), 4)

    def test_load_pos(self):
        self.assertAlmostEqual(self.item.loadPos(3), QPointF(699.69785575048729,188.73355263157893))

    def test_set_pos(self):
        self.assertTrue(self.item.submitPos(QPointF(100, 200), 0))
